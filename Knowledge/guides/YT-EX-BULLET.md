# YT-EX-BULLET · 防爆枪机 使用指南

> 产品形态：防爆（Explosion-proof）　|　分辨率：4K (3840×2160)　|　镜头：4mm 定焦
> 适用 App：影腾运维台　|　供电方式：DC24V（防爆接线）

## 一、产品简介

防爆枪机（型号 **YT-EX-BULLET**）属于影腾 防爆（Explosion-proof） 产品线，
主要面向石化、油库、加油站、粉尘车间等易燃易爆危险区域。
...
2. 选点定位：不锈钢防爆壳体，需由持证电工按防爆规范接线并可靠接地。
3. 固定机身：用随附膨胀螺丝固定支架，装好后调整镜头朝向并锁紧云台/万向节。
4. 连接线缆：按"供电方式（DC24V（防爆接线））"接通电源；网络型号用网线接入交换机或 NVR 的 PoE 口。
...
5. 激活成功后，可在"影腾运维台"中按型号 **YT-EX-BULLET** 纳管，统一查看在线状态与告警。
...
| 现象 | 可能原因 | 处理建议 |
| --- | --- | --- |
| 设备离线 | 供电中断 / 网络不通 | 检查DC24V（防爆接线）供电与网线，重启设备后在 App 重新连接 |
...
- 出现持续告警或无法自愈的故障时，联系影腾客服并提供型号 **YT-EX-BULLET** 与设备编号。
